...
class DooPlayer{
	...
	public function checkTv( $post ) {
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$superembed = '/wp-content/themes/dooplay/se_player.php?video_id='.$ids.'&tmdb=1&s='.$post['temporada'].'&e='.$post['episodio'] ;
		}
	...
	public function checkForMovie( $post ) {
		...
		return '/wp-content/themes/dooplay/se_player.php?video_id='.$post['ids'];
